...
interface Brief {
  ...
  participants: number; status: '진행중' | '종료';
...
const BRIEFS: Brief[] = [
  { id: '1', companyName: '카페인 (Cafe-In)', title: 'MZ세대를 위한 새로운 디카페인 커피 브랜드 런칭', problem: '기존 디카페인 커피는 "맛이 없다"는 편견이 강해 젊은 층의 유입이 적음.', target: '커피를 좋아하지만 카페인 민감도가 높은 2030 직장인 및 대학생', campaignInfo: '맛과 향을 모두 잡은 스위스 워터 프로세스 공법 강조', reward: '₩ 1,000,000', deadline: '3일 후 마감', participants: 248, status: '진행중', category: 'F&B', bgColor: 'bg-amber-50' },
  { id: '2', companyName: '그린웨이브 (GreenWave)', title: '친환경 세탁 서비스 "에코워시" 캠페인', problem: '편리함 위주의 세탁 서비스 시장에서 "환경 보호"라는 가치가 비용 부담으로 느껴지는 문제.', target: '지속 가능한 소비를 지향하고 가치 소비를 즐기는 1인 가구', campaignInfo: '미세 플라스틱 저감 세탁망 및 생분해 세제 사용 어필', reward: '₩ 500,000', deadline: '7일 후 마감', participants: 134, status: '진행중', category: '라이프스타일', bgColor: 'bg-green-50' },
  { id: '3', companyName: '핀트 (Fint)', title: '신규 핀테크 서비스 메인 카피 공모', problem: '금융 앱이 딱딱하고 어렵다는 인식을 깨고 싶음.', target: '재테크에 관심 있는 2030 초보 투자자', campaignInfo: '쉽고 친근한 투자 경험을 강조, 앱의 UI/UX 연계', reward: '₩ 2,500,000', deadline: '14일 후 마감', participants: 512, status: '진행중', category: '핀테크', bgColor: 'bg-blue-50' },
  { id: '4', companyName: '카페인 (Cafe-In)', title: '더 맑은 카페인, 세상을 깨우는 향', problem: '프리미엄 커피 시장에서 차별화된 브랜드 아이덴티티 확립 필요.', target: '고품질 커피를 추구하는 30~40대 직장인', campaignInfo: '원두 산지 강조, 핸드드립 공정 어필', reward: '₩ 1,000,000', deadline: '종료', participants: 1428, status: '종료', category: 'F&B', bgColor: 'bg-orange-50' },
  { id: '5', companyName: '에코워시', title: '에코워시: 지구를 닦는 스마트 세탁', problem: '친환경 세탁 서비스의 가격 부담 이미지 탈피.', target: '환경에 관심 있는 2030 1인 가구', campaignInfo: '생분해 세제, 미세플라스틱 저감 어필', reward: '₩ 500,000', deadline: '종료', participants: 842, status: '종료', category: '라이프스타일', bgColor: 'bg-emerald-50' },
  { id: '6', companyName: '핀테크 스타트업', title: '신규 핀테크 서비스 네이밍', problem: '복잡한 금융을 쉽게 접근하게 하는 서비스명 필요.', target: '20~30대 초보 투자자', campaignInfo: '직관적이고 친근한 이름 선호', reward: '₩ 2,500,000', deadline: '종료', participants: 3102, status: '종료', category: '핀테크', bgColor: 'bg-indigo-50' },
  { id: '7', companyName: 'MZ패션', title: 'MZ세대 취향 저격 패키징 슬로건', problem: '패키징이 구매 결정에 미치는 영향 강화 필요.', target: '트렌드에 민감한 18~28세', campaignInfo: '감각적이고 공유하고 싶은 카피 원함', reward: '₩ 700,000', deadline: '종료', participants: 612, status: '종료', category: '패션', bgColor: 'bg-pink-50' },
  { id: '8', companyName: '그린클로스', title: '친환경 소재 의류 캠페인 카피', problem: '친환경 패션의 가격 저항 극복.', target: '가치 소비를 추구하는 2030', campaignInfo: '지속가능성과 스타일 두 마리 토끼 강조', reward: '₩ 1,200,000', deadline: '종료', participants: 921, status: '종료', category: '패션', bgColor: 'bg-teal-50' },
];
...
function BriefCard({ brief, onClick }: { brief: Brief; onClick: () => void }) {
  return (
    <motion.div whileHover={{ scale: 1.02 }} onClick={onClick}
      ...
      {/* 이미지 영역 */}
      <div className={cn("relative h-52 rounded-2xl overflow-hidden mb-3", brief.bgColor)}>
      ...
      <div>
        <div className="flex items-start justify-between gap-2 mb-1">
          <h4 className="text-sm font-black leading-snug line-clamp-2 group-hover:text-black">{brief.title}</h4>
        </div>
        <p className="text-xs text-gray-500 font-bold flex items-center gap-1 mb-1">
          <Building2 size={10} /> {brief.companyName}
        </p>
...
// ── 브리프 상세 모달 (에어비앤비 스타일) ──────────────────
function BriefModal({ brief, copies, onClose, onSubmit, onVote, newCopy, setNewCopy }: {
  brief: Brief; copies: CopyEntry[]; onClose: () => void;
  onSubmit: (e: React.FormEvent) => void;
  onVote: (id: string, delta: number) => void;
  newCopy: string; setNewCopy: (v: string) => void;
}) {
  const sorted = [...copies].filter(c => c.briefId === brief.id)
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 30 }}
        ...
        {/* 상단 컬러 배너 */}
        <div className={cn("relative h-40 rounded-t-3xl", brief.bgColor)}>
          ...
          <div className="absolute bottom-4 left-6 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black text-gray-500 uppercase tracking-widest">{brief.category}</p>
              <p className="text-sm font-black text-gray-800">{brief.companyName}</p>
            </div>
          </div>
        ...
        <div className="p-8">
          ...
          <div className="flex items-start justify-between gap-4 mb-6">
            <h2 className="text-xl font-black leading-snug">{brief.title}</h2>
          ...
          <div className="space-y-5 mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-gray-50 rounded-2xl">
                ...
                <p className="text-sm font-bold text-gray-700 leading-relaxed">{brief.campaignInfo}</p>
              </div>
            </div>
            {brief.status === '진행중' && (
              ...
              <div className="mb-6">
                ...
                <form onSubmit={onSubmit} className="relative">
                  <input type="text" value={newCopy} onChange={e => setNewCopy(e.target.value)}
                    ...
                    className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 pr-14 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-black/10 focus:border-black/20 transition-all" />
                  <button type="submit"
                    className={cn("absolute right-2 top-2 bottom-2 w-11 rounded-xl flex items-center justify-center transition-all",
                      newCopy.trim() ? "bg-black text-white" : "bg-gray-200 text-gray-400 pointer-events-none")}>
                    <Send size={15} />
                  </button>
                </form>
              </div>
                ...
                <div className="space-y-3">
                  {sorted.map((copy, idx) => (
                    <div key={copy.id}
                      ...
                      {idx === 0 && (
                        <span className="text-[10px] font-black text-[#00BE00] bg-[#00BE00]/10 px-2 py-0.5 rounded mb-2 inline-block">🥇 1위</span>
                      )}
...
export default function App() {
  ...
  }), [briefStatusFilter, activeBriefCat]);
  ...
  const homeBriefs = BRIEFS.filter(b => b.status === '진행중').slice(0, 3);
          ...
          <div className="flex items-center gap-10">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {[{ id: 'brief', label: '브리프' }, { id: 'community', label: '커뮤니티' }].map(tab => (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); setSelectedPost(null); setSelectedBrief(null); }}
      ...
      <AnimatePresence>
        {selectedBrief && (
          <BriefModal
            brief={selectedBrief}
            copies={copies}
            onClose={() => { setSelectedBrief(null); setNewCopy(''); }}
