...
import {
  ...
  ThumbsUp, Search, Send, Building2, ArrowLeft,
  Clock, Eye, Edit3, X, ChevronDown,
  BarChart2, Zap, Star
} from 'lucide-react';
...
interface Brief {
  ...
  target: string; campaignInfo: string; reward: string; deadline: string; participants: number;
}
interface CopyEntry {
  id: string; briefId: string; author: string; content: string; votes: number; createdAt: number;
}
interface Reply {
  id: string; commentId: string; author: string; avatar: string; content: string; likes: number; createdAt: number;
}
interface Comment {
  id: string; postId: string; author: string; avatar: string; content: string; likes: number; createdAt: number; replies: Reply[];
}
interface Post {
  id: string; category: string; title: string; content: string; author: string; avatar: string;
  views: number; likes: number; createdAt: number; isPinned?: boolean; comments: Comment[];
}

const BRIEFS: Brief[] = [
  { id: '1', companyName: '카페인 (Cafe-In)', title: 'MZ세대를 위한 새로운 디카페인 커피 브랜드 런칭', problem: '기존 디카페인 커피는 "맛이 없다"는 편견이 강해 젊은 층의 유입이 적음.', target: '커피를 좋아하지만 카페인 민감도가 높은 2030 직장인 및 대학생', campaignInfo: '맛과 향을 모두 잡은 스위스 워터 프로세스 공법 강조', reward: '₩ 1,000,000', deadline: '3일 후 마감', participants: 248 },
  { id: '2', companyName: '그린웨이브 (GreenWave)', title: '친환경 세탁 서비스 "에코워시" 캠페인', problem: '편리함 위주의 세탁 서비스 시장에서 "환경 보호"라는 가치가 비용 부담으로 느껴지는 문제.', target: '지속 가능한 소비를 지향하고 가치 소비를 즐기는 1인 가구', campaignInfo: '미세 플라스틱 저감 세탁망 및 생분해 세제 사용 어필', reward: '₩ 500,000', deadline: '7일 후 마감', participants: 134 },
  { id: '3', companyName: '핀트 (Fint)', title: '신규 핀테크 서비스 메인 카피 공모', problem: '금융 앱이 딱딱하고 어렵다는 인식을 깨고 싶음.', target: '재테크에 관심 있는 2030 초보 투자자', campaignInfo: '쉽고 친근한 투자 경험을 강조, 앱의 UI/UX 연계', reward: '₩ 2,500,000', deadline: '14일 후 마감', participants: 512 },
];

const INITIAL_COPIES: CopyEntry[] = [
  { id: 'c1', briefId: '1', author: '카피왕', content: '밤의 카페인은 덜어내고, 아침의 햇살만 남겼습니다.', votes: 42, createdAt: Date.now() - 3600000 },
  { id: 'c2', briefId: '1', author: '문장가', content: '맛은 그대로, 불안은 없이.', votes: 38, createdAt: Date.now() - 7200000 },
  { id: 'c3', briefId: '1', author: 'wordsmiths', content: '당신의 밤을 지켜주는 커피.', votes: 21, createdAt: Date.now() - 10800000 },
  { id: 'c4', briefId: '2', author: 'EcoWriter', content: '세탁기 한 번에, 지구 한 번 더.', votes: 55, createdAt: Date.now() - 3600000 },
];

const INITIAL_POSTS: Post[] = [
  {
    id: 'p1', category: '팁&노하우', title: '현직 카피라이터가 알려주는 아이디어 발상법 10가지',
    content: '좋은 카피는 제품이 아닌 사람에서 시작됩니다.\n\n1. 타깃의 하루를 상상하라\n제품을 쓰는 사람의 아침부터 밤까지를 떠올려보세요. 언제, 어디서, 어떤 감정 상태에서 이 제품을 쓰는지가 카피의 출발점입니다.\n\n2. 제품의 기능이 아닌 변화를 써라\n"방수 기능"이 아니라 "비 오는 날도 걱정 없는 하루"가 더 강합니다.\n\n3. 말하지 말고 보여줘라\n"혁신적인 제품입니다"는 약하고, "처음 써본 날, 두 번 다시 전 제품 쓰고 싶지 않았습니다"는 강합니다.\n\n4. 숫자를 구체적으로\n"많은 사람"보다 "127만 명"이 더 믿음직합니다.\n\n5. 적의 말을 빌려라\n가장 강한 카피는 소비자가 실제로 하는 말에서 나옵니다. 리뷰를 읽으세요.',
    author: 'CopyMaster', avatar: 'CM', views: 1240, likes: 82, createdAt: Date.now() - 86400000 * 2, isPinned: true,
    comments: [
      { id: 'cm1', postId: 'p1', author: 'Creative_Lee', avatar: 'CL', content: '5번이 진짜 핵심인 것 같아요. 리뷰에서 카피를 찾는다는 발상이 신선합니다!', likes: 12, createdAt: Date.now() - 3600000, replies: [{ id: 'r1', commentId: 'cm1', author: 'CopyMaster', avatar: 'CM', content: '맞아요! 실제로 아마존 리뷰에서 헤드라인 뽑는 카피라이터들도 많습니다 ㅎㅎ', likes: 8, createdAt: Date.now() - 1800000 }] },
      { id: 'cm2', postId: 'p1', author: 'BrandGuru', avatar: 'BG', content: '3번이 너무 공감돼요. 저도 항상 "보여줘라"를 모토로 씁니다.', likes: 7, createdAt: Date.now() - 7200000, replies: [] }
    ]
  },
  {
    id: 'p2', category: '자유 게시판', title: '첫 수주 성공했습니다! 캐치카피 덕분에 인생이 바뀌네요.',
    content: '안녕하세요! 오늘 처음으로 브랜드 카피 수주에 성공했습니다.\n\n디카페인 커피 브리프에서 제 카피가 1등을 해서 상금 100만원을 받게 됐어요. 사실 카피라이팅에는 완전 초보였는데, 이 플랫폼에서 여러 브리프에 도전하면서 실력이 많이 늘었습니다.\n\n앞으로도 열심히 참여할게요! 다들 도전해보세요 :)',
    author: 'Beginner_Win', avatar: 'BW', views: 3100, likes: 420, createdAt: Date.now() - 86400000,
    comments: [{ id: 'cm3', postId: 'p2', author: 'NamingKing', avatar: 'NK', content: '축하드려요!! 저도 처음엔 계속 떨어지다가 10번째 도전에서 처음 수주했어요. 포기하지 않으면 다 됩니다!', likes: 34, createdAt: Date.now() - 3600000, replies: [{ id: 'r2', commentId: 'cm3', author: 'Beginner_Win', avatar: 'BW', content: '감사합니다!! 앞으로도 열심히 하겠습니다 🙏', likes: 5, createdAt: Date.now() - 1800000 }] }]
  },
  {
    id: 'p3', category: '크리에이터 토크', title: '브랜딩에서 가장 중요한 건 로고가 아니라 한 줄의 메시지입니다.',
    content: '요즘 스타트업들을 보면 로고에는 수백만원을 쓰면서 슬로건에는 아무 신경도 안 쓰는 경우가 많습니다.\n\n하지만 애플의 "Think Different", 나이키의 "Just Do It"처럼 강력한 한 줄은 로고보다 오래 기억됩니다.\n\n브랜드를 만들 때 가장 먼저 물어야 할 질문은 "우리 브랜드를 한 줄로 설명하면?"입니다.',
    author: 'BrandGuru', avatar: 'BG', views: 2500, likes: 142, createdAt: Date.now() - 86400000 * 3, comments: []
  },
  {
    id: 'p4', category: '질문&답변', title: '에코워시 브리프 참여하신 분 계신가요? 타겟 설정이 어렵네요.',
    content: '에코워시 브리프 도전 중인데 타겟 설정을 어떻게 해야 할지 모르겠어요.\n\n"1인 가구"라는 건 알겠는데, 카피를 너무 환경 중심으로 쓰면 딱딱하고 너무 편의 중심으로 쓰면 브랜드 아이덴티티가 흐려지는 것 같아서요.\n\n혹시 비슷한 고민 하신 분들 계신가요?',
    author: 'Creative_Lee', avatar: 'CL', views: 428, likes: 5, createdAt: Date.now() - 43200000,
    comments: [{ id: 'cm4', postId: 'p4', author: 'CopyMaster', avatar: 'CM', content: '"환경을 생각하는 나"라는 정체성에 초점을 맞춰보세요. 기능보다 자아 표현으로 가는 거예요.', likes: 18, createdAt: Date.now() - 3600000, replies: [] }]
  },
];
...
const getAvatarColor = (s: string) => {
  if (!AVATAR_COLORS[s]) AVATAR_COLORS[s] = PALETTE[s.charCodeAt(0) % PALETTE.length];
  return AVATAR_COLORS[s];
};

function Avatar({ name, size = 8 }: { name: string; size?: number }) {
  return (
    <div className={cn(`w-${size} h-${size} rounded-full flex items-center justify-center text-white text-xs font-black shrink-0`, getAvatarColor(name))}>
    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-black">{localPost.author}</p>
              <p className="text-xs text-gray-400">{timeAgo(localPost.createdAt)}</p>
            </div>
          </div>
      ...
      <div className="mb-12">
        {localPost.content.split('\n').map((line, i) => (
          <p key={i} className={cn("text-[15px] leading-relaxed text-gray-700", line === '' ? 'h-4' : 'mb-1')}>{line || ''}</p>
        ))}
      </div>
        ...
        <form onSubmit={handleComment} className="flex gap-3 mb-10">
          ...
          <div className="flex-1 relative">
            <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="댓글을 입력하세요." rows={3}
              className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 text-sm font-medium resize-none focus:outline-none focus:ring-2 focus:ring-black/10 focus:border-black/20 transition-all" />
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex justify-end gap-2 mt-2">
                          ...
                          <button onClick={() => handleReply(comment.id)} disabled={!replyContent.trim()}
                            className="px-4 py-1.5 bg-black text-white text-xs font-black rounded-xl disabled:opacity-30 disabled:pointer-events-none hover:bg-gray-800 transition-colors">등록</button>
                        </div>
                      </div>
                    </div>
                  )}
...
export default function App() {
  ...
  const [copies, setCopies] = useState<CopyEntry[]>(() => { try { return JSON.parse(localStorage.getItem('cc_copies') || ''); } catch { return INITIAL_COPIES; } });
  ...
  const sortedCopies = useMemo(() => [...copies].filter(c => c.briefId === selectedBrief?.id).sort((a, b) => b.votes - a.votes), [copies, selectedBrief]);
  ...
  const handleVote = (id: string) => setCopies(prev => prev.map(c => c.id === id ? { ...c, votes: c.votes + 1 } : c));

  const handleSubmitCopy = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCopy.trim() || !selectedBrief) return;
    setCopies(prev => [{ id: Math.random().toString(36).substr(2,9), briefId: selectedBrief.id, author: '나', content: newCopy, votes: 0, createdAt: Date.now() }, ...prev]);
    setNewCopy('');
  };
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {[{ id: 'brief', label: '브리프' }, { id: 'community', label: '커뮤니티' }].map(tab => (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); setSelectedPost(null); }}
                  className={cn("px-4 py-1.5 rounded-lg text-sm font-bold transition-all", activeTab === tab.id ? "bg-gray-100 text-black" : "text-gray-400 hover:text-black hover:bg-gray-50")}>
                  {tab.label}
                </button>
              ))}
            </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { if (activeTab === 'community') setShowWriteModal(true); }}
            ...
            <section className="max-w-7xl mx-auto px-6 py-14">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-7 space-y-4">
                  ...
                  {BRIEFS.map(brief => (
                    <motion.div key={brief.id} whileHover={{ x: 4 }} onClick={() => setSelectedBrief(brief)}
                      className={cn("p-6 rounded-2xl border cursor-pointer transition-all", selectedBrief?.id === brief.id ? "bg-white border-black shadow-lg" : "bg-gray-50 border-transparent hover:bg-white hover:border-gray-200 hover:shadow-md")}>
                ...
                <div className="lg:col-span-5">
                  <AnimatePresence mode="wait">
                    {selectedBrief ? (
                      <motion.div key={selectedBrief.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
                        className="bg-white rounded-2xl border border-gray-200 shadow-lg p-8 lg:sticky lg:top-20">
                        <div className="mb-6 pb-6 border-b border-gray-100">
                          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">선택된 브리프</p>
                          <h3 className="text-lg font-black mb-4">{selectedBrief.companyName}</h3>
                          <div className="space-y-3">
                            <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">미션</p><p className="text-sm font-bold text-gray-800 leading-relaxed">{selectedBrief.problem}</p></div>
                            <div className="p-4 bg-gray-50 rounded-xl"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">타겟</p><p className="text-sm font-bold text-gray-700">{selectedBrief.target}</p></div>
                          </div>
                        </div>
                        <form onSubmit={handleSubmitCopy} className="relative mb-6">
                          <input type="text" value={newCopy} onChange={e => setNewCopy(e.target.value)} placeholder="당신의 한 줄을 제안하세요."
                          ...
                          <button type="submit" className={cn("absolute right-2 top-2 bottom-2 w-10 rounded-lg flex items-center justify-center transition-all", newCopy.trim() ? "bg-black text-white" : "bg-gray-200 text-gray-400 pointer-events-none")}><Send size={14}/></button>
                        </form>
                        <div>
                          <div className="flex items-center gap-2 mb-4"><BarChart2 size={14} className="text-gray-400"/><p className="text-xs font-black text-gray-400 uppercase tracking-widest">실시간 랭킹</p></div>
                          <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                            {sortedCopies.length === 0 ? (
                              <div className="text-center py-8"><p className="text-sm font-bold text-gray-300">첫 번째 카피를 제출해보세요!</p></div>
                            ) : sortedCopies.map((copy, idx) => (
                              <div key={copy.id} className={cn("p-4 rounded-xl border", idx === 0 ? "bg-gray-50 border-gray-200" : "bg-white border-gray-100")}>
                                <div className="flex items-center gap-3">
                                  <span className={cn("w-6 h-6 rounded-full flex items-center justify-center text-xs font-black shrink-0", idx === 0 ? "bg-black text-white" : "bg-gray-100 text-gray-500")}>{idx+1}</span>
                                  <p className="text-sm font-bold flex-1 leading-snug text-gray-800">"{copy.content}"</p>
                                  <button onClick={() => handleVote(copy.id)} className="flex items-center gap-1 text-xs font-black text-gray-400 hover:text-black transition-colors shrink-0"><ThumbsUp size={12}/> {copy.votes}</button>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="border-2 border-dashed border-gray-200 rounded-2xl h-80 flex flex-col items-center justify-center text-gray-300 lg:sticky lg:top-20">
        ...
        {activeTab === 'brief' && (
          <div className="max-w-7xl mx-auto px-6 py-14">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { title: '더 맑은 카페인, 세상을 깨우는 향', reward: '₩ 1,000,000', status: '종료', participants: '1,428' },
                { title: '에코워시: 지구를 닦는 스마트 세탁', reward: '₩ 500,000', status: '진행중', participants: '842' },
                { title: '신규 핀테크 서비스 네이밍', reward: '₩ 2,500,000', status: '종료', participants: '3,102' },
                { title: 'MZ세대 취향 저격 패키징 슬로건', reward: '₩ 700,000', status: '종료', participants: '612' },
                { title: '친환경 소재 의류 캠페인 카피', reward: '₩ 1,200,000', status: '종료', participants: '921' },
                { title: 'IT 스타트업 서비스 메인 카피', reward: '₩ 1,500,000', status: '진행중', participants: '1,154' },
              ].map((item, idx) => (
                <motion.div key={idx} whileHover={{ y: -4 }} className="bg-white rounded-2xl border border-gray-200 overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-300">
                  <div className="h-28 bg-gray-50 flex items-center justify-center"><Trophy size={28} className="text-gray-200"/></div>
                  <div className="p-6">
                    <span className={cn("text-[10px] font-black px-2.5 py-1 rounded-full mb-3 inline-block", item.status === '진행중' ? "bg-[#00BE00]/10 text-[#00BE00]" : "bg-gray-100 text-gray-400")}>{item.status}</span>
                    <h4 className="text-base font-black mb-4 leading-snug">{item.title}</h4>
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      <p className="text-lg font-black">{item.reward}</p>
                      <p className="text-xs font-bold text-gray-400 flex items-center gap-1"><Users size={11}/> {item.participants}명</p>
                    </div>
                  </div>
                </motion.div>
              ))}
                    ...
                    <div className="space-y-0.5">
                      {CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => setActiveCategory(cat)}
                          className={cn("w-full text-left px-4 py-2.5 rounded-xl text-sm font-bold transition-all", activeCategory === cat ? "bg-white border border-gray-200 text-black shadow-sm" : "text-gray-500 hover:text-black hover:bg-white")}>
                          {cat}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="검색"
                          className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold w-40 focus:outline-none focus:ring-2 focus:ring-black/10 focus:border-black/20 transition-all pr-8" />
                        <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" size={13}/>
                      </div>
                      <button onClick={() => setShowWriteModal(true)} className="flex items-center gap-1.5 bg-black text-white text-sm font-black px-4 py-2 rounded-xl hover:bg-gray-800 transition-colors">
                        <Edit3 size={13}/> 글쓰기
                      </button>
                    </div>
                  </div>
