...
function LoginPage() {
  ...
  const ADMIN_PW = 'catchcopy2024';

  const handleLogin = (provider: 'naver' | 'kakao') => {
    const profile = { name: provider === 'naver' ? '네이버유저' : '카카오유저', avatar: provider === 'naver' ? 'NV' : 'KK' };
    setUser(profile);
    localStorage.setItem('cc_user', JSON.stringify(profile));
    localStorage.setItem('cc_admin', '0');
    navigate(-1);
  };

  const handleAdminLogin = () => {
    if (adminPw !== ADMIN_PW) { alert('비밀번호가 틀렸습니다.'); return; }
    const profile = { name: '관리자', avatar: 'AD' };
    setUser(profile);
    localStorage.setItem('cc_user', JSON.stringify(profile));
            ...
            <div className="mt-2 space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleAdminLogin}
                  className="flex-1 py-2 rounded-lg text-xs font-semibold text-white hover:opacity-90 transition-all"
  ...
  return (
    <DarkWrapper className="min-h-screen pt-12 pb-24 lg:pb-0">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-4">
        ...
        {isAdminMode() && (
          <div className="rounded-2xl" style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6' }}>
            <p className="text-[10px] font-semibold uppercase tracking-wider px-5 pt-5 pb-3" style={{ color: D.muted }}>관리자 메뉴</p>
            {[
              { label: '브리프 관리', sub: '브리프 등록·수정·삭제', path: '/catchcopy/admin' },
              { label: '새 브리프 등록', sub: '새로운 브리프 작성', path: '/catchcopy/admin/brief/write' },
              { label: '지급 대기 관리', sub: 'TOP4 캐치 지급 처리', path: '/catchcopy/admin' },
            ].map((item, i) => (
              <button key={i} onClick={() => navigate(item.path)}
                className="w-full flex items-center justify-between px-5 py-3.5 border-t text-left"
                style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                <div>
                  <p className="text-sm font-medium" style={{ color: dm ? D.text : '#111' }}>{item.label}</p>
                  <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>{item.sub}</p>
                </div>
                <ChevronRight size={14} style={{ color: D.muted }} />
              </button>
            ))}
          </div>
        )}
...
function AdBanner() {
  ...
  useEffect(() => {
    supabase.from('ads').select('*').eq('is_active', true).order('sort_order').then(({ data }) => {
      setAds(data ?? []);
    });
  }, []);
  ...
  return (
    <div className="w-full relative overflow-hidden" style={{ height: 'calc(var(--brief-card-height, 220px) * 0.8)', minHeight: 140, maxHeight: 200, cursor: ad.link_url ? 'pointer' : 'default' }}
      onClick={handleClick}>
      {ads.map((a, idx) => (
        <motion.div key={a.id}
          initial={false}
          ...
          <img src={a.image_url} alt="광고" className="w-full h-full object-cover" draggable={false} />
        </motion.div>
      ))}
      {/* 인디케이터 */}
      {ads.length > 1 && (
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1.5 z-10">
          {ads.map((_, idx) => (
            <button key={idx} onClick={e => { e.stopPropagation(); setCurrent(idx); clearInterval(timer.current); }}
...
function MobileTabBar() {
  ...
  const activeTab = path.startsWith('/catchcopy/brief') && !path.includes('/admin') ? 'brief'
    ...
    : path.startsWith('/catchcopy/wallet') ? 'wallet'
    : path.startsWith('/catchcopy/mypage') ? 'mypage'
    : path.startsWith('/catchcopy/market') ? 'wallet'
    : 'home';
  ...
  const tabs = [
    ...
    { id: 'brief', label: '브리프', path: '/catchcopy/brief',
    ...
    { id: 'community', label: '커뮤니티', path: '/catchcopy/community',
    ...
    { id: 'wallet', label: '내 지갑', path: '/catchcopy/wallet',
      icon: (a: boolean) => <svg width="26" height="26" viewBox="0 0 24 24" fill={a ? KEY : (dm ? D.muted : '#9ca3af')}><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05.82 1.87 2.65 1.87 1.96 0 2.4-.98 2.4-1.59 0-.83-.44-1.61-2.67-2.14-2.48-.6-4.18-1.62-4.18-3.67 0-1.72 1.39-2.84 3.11-3.21V4h2.67v1.95c1.86.45 2.79 1.86 2.85 3.39H14.3c-.05-1.11-.64-1.87-2.22-1.87-1.5 0-2.4.68-2.4 1.64 0 .84.65 1.39 2.67 1.91s4.18 1.39 4.18 3.91c-.01 1.83-1.38 2.83-3.12 3.16z"/></svg> },
    { id: 'mypage', label: '마이페이지', path: '/catchcopy/mypage',
      ...
      <div className="max-w-6xl mx-auto px-4 h-12 flex items-center justify-between">
        <div className="flex items-center gap-5">
          ...
          <nav className="hidden md:flex items-center gap-0.5">
            {[{ id: 'brief', label: '브리프' }, { id: 'community', label: '커뮤니티' }, { id: 'wallet', label: '내 지갑' }, { id: 'market', label: '캐치마켓' }].map(t => (
        ...
        <div className="flex items-center gap-1.5">
          {user ? (
            <button onClick={() => navigate('/catchcopy/mypage')}
              ...
              style={{ color: dm ? D.text : '#374151', borderColor: isAdminMode() ? KEY : (dm ? D.border : '#e5e7eb'), backgroundColor: dm ? D.hover : '#f9fafb' }}>
              {user.avatarUrl
...
function BriefPage() {
  ...
  const handleSubmitCopy = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Edge Function으로 서버사이드 처리 (카피 제출 + 캐치 적립)
      const result = await callEdgeFunction('submit-copy', {
        voter_key: vk,
        brief_id: briefId,
        content: newCopy.trim(),
        author: user?.name ?? '익명',
      });
      setCatches(result.catches);
      localStorage.setItem('cc_catches', String(result.catches));
      setHasCopy(true);
      setNewCopy('');
  ...
  if (mode === 'read') {
    return (
      <div className="min-h-screen pt-12 pb-32" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
        {/* 풀스크린 헤더 이미지 */}
        <div className={cn("relative w-full", !brief.image_url && brief.bg_color)} style={{ height: '40vh', minHeight: 220 }}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
            ...
            <p className="text-xs text-white/60">{brief.company_name} · {brief.reward}</p>
          </div>
        </div>

        {/* 브리프 내용 전체 */}
        <div className="max-w-2xl mx-auto px-4 py-8 space-y-6" ref={contentRef}>
          {[
            ...
            <div key={label}>
              ...
              <p className="text-sm leading-relaxed" style={{ color: dm ? D.text : '#374151' }}>{content}</p>
            </div>
          ))}
          ...
          <div className="rounded-2xl p-5 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: D.muted }}>상금</p>
                <p className="text-lg font-bold" style={{ color: dm ? D.text : '#111' }}>{brief.reward}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: D.muted }}>참여자</p>
                <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>{brief.participants.toLocaleString()}명</p>
              </div>
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: D.muted }}>카피 수</p>
                <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>{copies.length}개</p>
              </div>
            </div>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 border-t"
          ...
          <div className="max-w-2xl mx-auto space-y-2">
            {!hasRead && lockSeconds === 0 ? (
              <button onClick={startReadLock}
                className="w-full py-4 rounded-xl text-sm font-bold text-white transition-all active:scale-95"
                style={{ backgroundColor: KEY }}>
                브리프 읽기 시작 →
              </button>
            ) : lockSeconds > 0 ? (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-medium" style={{ color: D.muted }}>브리프를 읽는 중... {lockSeconds}초 남음</p>
                  <span className="text-xs font-bold" style={{ color: KEY }}>{readProgress}%</span>
                </div>
                <div className="w-full h-1.5 rounded-full mb-3 overflow-hidden" style={{ backgroundColor: dm ? D.hover : '#f3f4f6' }}>
                  <div className="h-full rounded-full transition-all duration-1000" style={{ width: readProgress + '%', backgroundColor: KEY }} />
                </div>
                <button disabled
                  className="w-full py-4 rounded-xl opacity-30 cursor-not-allowed flex items-center justify-center gap-2"
                  style={{ backgroundColor: KEY }}>
                  <img src="/button.png" alt="LET'S CATCHCOPY" style={{ height: 22, width: 'auto' }} />
                  <span className="text-white text-xs font-medium">({lockSeconds}초 후 활성화)</span>
                </button>
              </div>
            ) : canParticipate ? (
              <button onClick={handleGoParticipate}
                className="w-full py-4 rounded-xl transition-all active:scale-95 hover:opacity-90 flex items-center justify-center"
                style={{ backgroundColor: KEY }}>
                <img src="/button.png" alt="LET'S CATCHCOPY" style={{ height: 22, width: 'auto' }} />
              </button>
            ) : (
              <div className="space-y-2">
                <p className="text-center text-xs" style={{ color: D.muted }}>
                  자세히 알아보기 버튼을 눌러 상세 내용을 확인 후 멋진 카피를 작성해주세요!
                </p>
                <button disabled
                  className="w-full py-4 rounded-xl opacity-30 cursor-not-allowed flex items-center justify-center"
                  style={{ backgroundColor: KEY }}>
                  <img src="/button.png" alt="LET'S CATCHCOPY" style={{ height: 22, width: 'auto' }} />
                </button>
              </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
          ...
          <div className="flex items-end justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-xs text-white/60">{brief.participants.toLocaleString()}명 참여</p>
            </div>
          </div>
          {!getDday(brief.deadline).isExpired && brief.status !== '종료' && (
            <p className="text-xs text-white/70 mt-2 flex items-center gap-1"><Clock size={11} /> {getDday(brief.deadline).label}</p>
          )}
          {(getDday(brief.deadline).isExpired || brief.status === '종료') && (
            <p className="text-xs text-white/60 mt-2 flex items-center gap-1"><Clock size={11} /> 브리프 마감</p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>해결할 문제</p>
              <p className="text-sm leading-relaxed" style={{ color: dm ? D.text : '#374151' }}>{brief.problem}</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>타겟</p>
                <p className="text-xs leading-relaxed" style={{ color: dm ? D.text : '#374151' }}>{brief.target}</p>
              </div>
              <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>캠페인</p>
                <p className="text-xs leading-relaxed" style={{ color: dm ? D.text : '#374151' }}>{brief.campaign_info}</p>
              </div>
            </div>
            {!getDday(brief.deadline).isExpired && brief.status !== '종료' && (
              <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: D.muted }}>내 카피 제출</p>
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(5,213,96,0.1)', color: KEY }}>+400 캐치</span>
                </div>
                {hasCopy ? (
                  <p className="text-xs py-2" style={{ color: D.muted }}>✅ 이 브리프에 카피를 작성하셨습니다.</p>
                ) : (
                  <form onSubmit={handleSubmitCopy} className="relative">
                    <input type="text" value={newCopy} onChange={e => setNewCopy(e.target.value)}
                      ...
                      className="w-full rounded-lg px-3 py-2.5 pr-10 text-sm focus:outline-none focus:border-[#05D560] transition-colors border"
                      style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }} />
                    <button type="submit"
            ...
            <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                <div className="flex items-center gap-2">
                  <BarChart2 size={14} style={{ color: D.muted }} />
                  <span className="text-sm font-semibold" style={{ color: dm ? D.text : '#1f2937' }}>
                    {(brief.status === '종료' || getDday(brief.deadline).isExpired) ? '최종 순위' : '실시간 경쟁 랭킹'}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs" style={{ color: D.muted }}>{copies.length}개 참여</span>
                  {copies.length > 0 && !(brief.status === '종료' || getDday(brief.deadline).isExpired) && (
                    <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: KEY }} />
                  )}
                </div>
              </div>
              {/* 종료 시 수상 축하 배너 */}
              {(brief.status === '종료' || getDday(brief.deadline).isExpired) && copies.length >= 4 && (
                <div className="px-4 py-3 text-center border-b" style={{ backgroundColor: 'rgba(251,191,36,0.08)', borderColor: dm ? D.border : '#f9fafb' }}>
                  <p className="text-sm font-bold" style={{ color: '#f59e0b' }}>🏆 수상을 축하합니다!</p>
                  <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>TOP 4 수상자에게 캐치가 지급될 예정입니다.</p>
                </div>
              )}
              {loading ? <Spinner /> : copies.length === 0 ? (
                <div className="text-center py-12">
                  <p className="text-2xl mb-2">✍️</p>
                  <p className="text-xs" style={{ color: D.muted }}>첫 번째 카피를 제출해보세요!</p>
                </div>
              ) : (
                <div className="divide-y" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                  {copies.map((copy, idx) => {
                    ...
                    const isFinished = brief.status === '종료' || getDday(brief.deadline).isExpired;
                    const isWinner = isFinished && isTop4;
                    return (
                      <motion.div key={copy.id} layout
                        className="px-4 py-3.5 flex items-start gap-3 transition-colors relative"
                        style={{
                          backgroundColor: isWinner
                            ? 'rgba(251,191,36,0.06)'
                            : idx === 0 ? 'rgba(5,213,96,0.03)' : 'transparent',
                          boxShadow: isWinner ? 'inset 0 0 0 1px rgba(251,191,36,0.2)' : 'none',
                        }}>
                        {/* 황금빛 glow 효과 */}
                        {isWinner && (
                          <div className="absolute left-0 top-0 bottom-0 w-0.5 rounded-full" style={{ backgroundColor: '#f59e0b' }} />
                        )}
                        <div className="shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold mt-0.5"
                          style={isWinner
                            ? { backgroundColor: '#f59e0b', color: 'white', boxShadow: '0 0 8px rgba(251,191,36,0.6)' }
                            : idx === 0 ? { backgroundColor: KEY, color: 'white' }
                              : idx === 1 ? { backgroundColor: '#9CA3AF', color: 'white' }
                                : idx === 2 ? { backgroundColor: '#D97706', color: 'white' }
                                  : { backgroundColor: dm ? D.hover : '#F3F4F6', color: dm ? D.muted : '#6B7280' }}>
                          {isTop3 ? MEDAL[idx] : idx + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium leading-snug break-words" style={{ color: dm ? D.text : '#1f2937' }}>"{copy.content}"</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <p className="text-[11px]" style={{ color: D.muted }}>{copy.author}</p>
                            {isWinner && (
                              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(251,191,36,0.15)', color: '#f59e0b' }}>
                                {['40%', '30%', '20%', '10%'][idx]} 수상
                              </span>
                            )}
                          </div>
                        ...
                        <button onClick={() => handleVote(copy.id)}
                          disabled={isFinished}
                          className="shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                          style={voted
                            ? { border: '1.5px solid #05D560', color: '#05D560', backgroundColor: 'rgba(5,213,96,0.05)' }
                            : { border: dm ? ('1px solid ' + D.border) : '1px solid #e5e7eb', color: '#9ca3af', backgroundColor: 'transparent' }}>
...
function BriefWritePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('briefs').insert({
      ...form, participants: 0, bg_color: 'bg-gray-200', image_url: image_url || null, external_url: form.external_url || null,
    });
    setSubmitting(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="relative">
                <select value={form.category} onChange={e => set('category', e.target.value)} className={inputClass + " appearance-none pr-7"} style={inputStyle}>
                  {BRIEF_CATS.filter(c => c !== '전체').map(c => <option key={c}>{c}</option>)}
                </select>
                <ChevronDown size={12} className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: D.muted }} />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={form.reward} onChange={e => set('reward', e.target.value)} placeholder="₩ 1,000,000" className={inputClass} style={inputStyle} />
            </div>
            <div>
...
function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'briefs' | 'payouts' | 'ads'>('briefs');
  const [ads, setAds] = useState<Ad[]>([]);
    ...
    <div className="min-h-screen bg-gray-50 pt-12">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-5" style={{ backgroundColor: dm ? D.card : '#f3f4f6' }}>
          {[{ id: 'briefs', label: '브리프 관리' }, { id: 'payouts', label: '지급대기 ' + payouts.filter(p => p.status === 'pending').length }, { id: 'ads', label: '광고 관리' }].map(t => (
        ...
        {activeTab === 'briefs' && (<>
        {/* 관리자 모드 토글 */}
        <div className="rounded-xl border p-4 mb-5 flex items-center justify-between" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
          <div>
            <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>커뮤니티 관리자 모드</p>
            <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>활성화 시 모든 게시글·댓글을 수정/삭제할 수 있습니다.</p>
          </div>
          <button onClick={toggleAdminMode}
            className="w-11 h-6 rounded-full relative transition-all duration-300 shrink-0"
            style={{ backgroundColor: adminMode ? KEY : '#e5e7eb' }}>
            <motion.div animate={{ x: adminMode ? 22 : 2 }}
              className="absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm" />
          </button>
        </div>
        {/* 요약 통계 */}
          ...
          <div>
            ...
            <div className="rounded-xl border p-4 mb-4 flex items-center justify-between"
              ...
              <div>
                ...
                <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>PNG 이미지 · 4초마다 자동 전환</p>
              </div>
              <div>
                <input ref={adFileRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={handleAdUpload} className="hidden" />
            ...
            {/* 광고 미리보기 */}
            {ads.length > 0 && (
              <div className="rounded-xl overflow-hidden mb-4 border" style={{ borderColor: dm ? D.border : '#f3f4f6' }}>
                <AdBanner />
              </div>
            )}
            ...
            {ads.length === 0 ? (
              <div className="text-center py-12" style={{ color: D.muted }}>
                <p className="text-xs">등록된 광고가 없습니다.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {ads.map((ad, idx) => (
                  <div key={ad.id} className="rounded-xl border overflow-hidden"
                    ...
                    <div className="flex items-center gap-3 p-3">
                      {/* 썸네일 */}
                      <img src={ad.image_url} alt="" className="w-20 h-14 object-cover rounded-lg shrink-0" />
                      <div className="flex-1 min-w-0">
                        {/* URL 입력 */}
                        <input
                          type="url"
                          ...
                          className="w-full text-xs rounded-lg px-2.5 py-1.5 border focus:outline-none focus:border-[#05D560] transition-colors"
                          style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }}
                        />
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-[10px]" style={{ color: D.muted }}>{idx + 1}번째</span>
                        </div>
                      </div>
                      {/* 순서/삭제 버튼 */}
                      <div className="flex flex-col gap-1 shrink-0">
                        <button onClick={() => handleAdOrder(ad.id, 'up')} disabled={idx === 0}
                          className="w-7 h-7 rounded-lg flex items-center justify-center border disabled:opacity-30 transition-all"
                          style={{ borderColor: dm ? D.border : '#e5e7eb', color: D.muted }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {[
            ...
            <div key={key}>
              <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">{label}</label>
              {multi
                ? <textarea value={(form as any)[key]} onChange={e => set(key, e.target.value)} placeholder={placeholder} rows={2} className={inputClass + " resize-none"} style={inputStyle} />
                : <input type="text" value={(form as any)[key]} onChange={e => set(key, e.target.value)} placeholder={placeholder} className={inputClass} style={inputStyle} />
              }
            </div>
          ))}
...
function WritePage() {
  ...
  return (
    <div className="min-h-screen pt-12 flex flex-col" style={{ backgroundColor: dm ? D.bg : 'white' }}>
      ...
      {loading ? (
        ...
        <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-6 flex flex-col">
          ...
          <div
            ...
            className="flex-1 outline-none text-sm leading-relaxed"
            style={{ minHeight: 'calc(100vh - 380px)', color: dm ? D.text : '#374151' }}
          />
        </div>
      )}

      <div className="sticky bottom-0 border-t z-40" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <div className="max-w-2xl mx-auto px-4 py-2 flex items-center gap-1">
          {autoSaved && !isEdit && (
            <span className="text-[10px] mr-1 transition-opacity" style={{ color: KEY }}>✓ 자동저장됨</span>
          )}
      ...
      <style>{`[contenteditable][data-placeholder]:empty:before{content:attr(data-placeholder);color:#d1d5db;pointer-events:none}`}</style>
    </div>
  );
}
...
const isAdminMode = () => {
  const lsAdmin = localStorage.getItem('cc_admin') === '1';
  try {
    const userStr = localStorage.getItem('cc_user');
    if (!userStr) return false;
    const u = JSON.parse(userStr);
    // 두 조건 모두 충족해야 관리자 (localStorage 하나만 조작해선 불가)
    return lsAdmin && u?.name === '관리자' && u?.avatar === 'AD';
  } catch { return false; }
};
...
function PostDetail() {
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    await supabase.from('comments').insert({ post_id: postId, author: user?.name ?? '익명', avatar: user?.avatar ?? 'AN', content: newComment.trim(), likes: 0 });
    setNewComment('');
  };
          ...
          <div className="p-5 sm:p-6">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-50">
              <div className="flex items-center gap-2.5">
                <Avatar name={post.avatar} size={8} imageUrl={user?.name === post.author ? user?.avatarUrl : undefined} />
                <div>
              ...
              <div className="flex items-center gap-3">
                ...
                {(isAdminMode() || (user && user.name === post.author) || (!user && false)) && (<>
            ...
            <div className="space-y-5">
              {sortedComments.map(comment => (
                <div key={comment.id} className="flex gap-3">
                  <Avatar name={comment.avatar} size={8} imageUrl={user?.name === comment.author ? user?.avatarUrl : undefined} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {/* 본인 댓글이거나 관리자만 수정/삭제 */}
                      {(isAdminMode() || (user && user.name === comment.author)) && (
                        <div className="flex items-center gap-2">
...
export default function App() {
  ...
  // 초기값 0 - DB에서 즉시 로드하므로 localStorage는 사용 안 함
  ...
  useEffect(() => {
    ...
    supabase.from('user_catches').select('catches').eq('voter_key', vk).single()
      .then(({ data }) => {
        if (data) {
          // DB값으로 localStorage 덮어쓰기 (변조된 localStorage 무력화)
          setCatches(data.catches);
          localStorage.setItem('cc_catches', String(data.catches));
        } else {
          // DB에 없으면 0으로 초기화 (localStorage 무시)
          setCatches(0);
  ...
  return (
    <AppContext.Provider value={{ user, setUser, darkMode, toggleDarkMode, catches, setCatches }}>
