...
function AppInner() {
  ...
  useEffect(() => {
    supabase.from('posts').select('*, comments(*, replies(*))').order('created_at', { ascending: false }).then(({ data }) => {
      ...
      // 5시간 이내 게시글 중 추천수 상위 5개
      const fiveHoursAgo = new Date(Date.now() - 5 * 60 * 60 * 1000);
      const hot = [...d]
        .filter(p => new Date(p.created_at) >= fiveHoursAgo)
        .sort((a, b) => (b.likes ?? 0) - (a.likes ?? 0))
        .slice(0, 5);
      // 5시간 내 글이 5개 미만이면 전체에서 추천 많은 순으로 채우기
      if (hot.length < 5) {
        const extra = [...d]
    ...
    const ch = supabase.channel('posts-rt')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'posts' }, async () => {
        ...
        setPosts(d);
        const fiveHoursAgo = new Date(Date.now() - 5 * 60 * 60 * 1000);
        const hot = [...d].filter(p => new Date(p.created_at) >= fiveHoursAgo).sort((a, b) => (b.likes ?? 0) - (a.likes ?? 0)).slice(0, 5);
        if (hot.length < 5) {
          const extra = [...d].filter(p => !hot.find(h => h.id === p.id)).sort((a, b) => (b.likes ?? 0) - (a.likes ?? 0)).slice(0, 5 - hot.length);
          setHotPosts([...hot, ...extra]);
      ...
      <main className="pt-12 flex-1">
        ...
        {activeTab === 'home' && (
          <div className="overflow-y-scroll" style={{ height: 'calc(100vh - 48px)', scrollSnapType: 'y mandatory', scrollBehavior: 'smooth' }}>
            {/* 히어로 */}
            <section className="bg-white border-b border-gray-100 px-4 text-center flex flex-col items-center justify-center" style={{ minHeight: 'calc(100vh - 48px)', scrollSnapAlign: 'start', scrollSnapStop: 'always' }}>
