  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: dm ? D.bg : '#f9fafb', color: dm ? D.text : '#111' }}>
      <main className="pt-12 flex-1">
        ...
        {/* ── COMMUNITY ── */}
        {activeTab === 'community' && (
          <div className="pb-20 lg:pb-0"> {/* 모바일 하단탭 여백 */}

            {/* 모바일 상단 컨트롤 */}
            <div className="sticky top-12 z-30 border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              {/* 카테고리 탭 */}
              <div className="flex overflow-x-auto scrollbar-hide px-4 pt-3 gap-0">
                {CATEGORIES.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)}
                    ...
                    style={{
                      color: activeCategory === cat ? KEY : dm ? D.muted : '#9ca3af',
                      borderBottomColor: activeCategory === cat ? KEY : 'transparent'
                    }}>
                    {cat}
                  </button>
                ))}
            ...
            {/* 메인 그리드 */}
            <div className="max-w-4xl mx-auto lg:px-4 lg:py-6 lg:grid lg:grid-cols-4 lg:gap-5">

              {/* 데스크톱 사이드바 */}
              <aside className="hidden lg:block lg:col-span-1">
                <div className="rounded-xl p-4 lg:sticky lg:top-28 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 lg:hidden border-t z-50"
        ...
        <div className="grid grid-cols-4 h-14">
          {[
            { id: 'home', label: '홈', icon: <Star size={20} /> },
            { id: 'brief', label: '브리프', icon: <Megaphone size={20} /> },
            { id: 'community', label: '커뮤니티', icon: <MessageSquare size={20} /> },
            { id: 'wallet', label: '내 지갑', icon: <Zap size={20} /> },
          ].map(t => (
            <button key={t.id} onClick={() => navigate(`/catchcopy/${t.id === 'home' ? '' : t.id}`)}
              className="flex flex-col items-center justify-center gap-1 transition-colors"
              style={{ color: activeTab === t.id ? KEY : D.muted }}>
              {t.icon}
