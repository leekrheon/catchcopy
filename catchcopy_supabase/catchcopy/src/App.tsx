      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/catchcopy/admin/brief/write')}
            className="hidden sm:block text-sm font-bold text-gray-400 hover:text-black px-3 py-1.5 transition-colors border border-gray-200 rounded-lg hover:border-gray-400">관리자</button>
...
// ── 글쓰기 / 수정 페이지 ──────────────────────────────────
// ── 브리프 작성 페이지 (관리자) ──────────────────────────
const BRIEF_BG_OPTIONS = [
  { label: '앰버', value: 'bg-amber-100' },
  { label: '블루', value: 'bg-blue-100' },
  { label: '그린', value: 'bg-green-100' },
  { label: '핑크', value: 'bg-pink-100' },
  { label: '퍼플', value: 'bg-purple-100' },
  { label: '인디고', value: 'bg-indigo-100' },
  { label: '오렌지', value: 'bg-orange-100' },
  { label: '틸', value: 'bg-teal-100' },
];
...
function BriefWritePage() {
  ...
  const [form, setForm] = useState({
    ...
    reward: '', deadline: '', category: 'F&B', bg_color: 'bg-amber-100', status: '진행중' as '진행중' | '종료',
  });
  const [submitting, setSubmitting] = useState(false);
  ...
  const inputClass = "w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-black/10 transition-all";
    ...
    <div className="min-h-screen bg-white pt-14">
      <div className="sticky top-14 z-40 bg-white border-b border-gray-100">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <h2 className="text-sm font-black">브리프 등록 (관리자)</h2>
          <button onClick={handleSubmit} disabled={submitting}
            className={cn("px-5 py-2 text-sm font-black rounded-xl transition-all",
              !submitting ? "bg-black text-white hover:bg-gray-800 active:scale-95" : "bg-gray-100 text-gray-400 cursor-not-allowed")}>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* 미리보기 배너 */}
          <div className={cn("relative h-32 rounded-2xl overflow-hidden flex items-center justify-center", form.bg_color)}>
            <div className="text-center">
              <p className="text-xs font-black text-gray-500 uppercase tracking-widest">{form.category || '카테고리'}</p>
              <p className="text-base font-black text-gray-800 mt-1">{form.company_name || '회사명'}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1.5">상태</label>
              <div className="flex bg-gray-100 p-1 rounded-xl gap-1">
                {(['진행중', '종료'] as const).map(s => (
                  <button key={s} type="button" onClick={() => set('status', s)}
                    className={cn("flex-1 py-2 rounded-lg text-xs font-black transition-all",
                      form.status === s ? "bg-white shadow-sm text-black" : "text-gray-500")}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          {[
            { key: 'company_name', label: '회사명', placeholder: '예: 카페인 (Cafe-In)' },
            { key: 'title', label: '브리프 제목', placeholder: '예: MZ세대를 위한 새로운 디카페인 커피 브랜드 런칭' },
            { key: 'problem', label: '해결할 문제', placeholder: '예: 기존 디카페인 커피는 맛이 없다는 편견이 강해 젊은 층의 유입이 적음.' },
            { key: 'target', label: '타겟', placeholder: '예: 커피를 좋아하지만 카페인 민감도가 높은 2030 직장인' },
            { key: 'campaign_info', label: '캠페인 방향', placeholder: '예: 맛과 향을 모두 잡은 스위스 워터 프로세스 공법 강조' },
          ].map(({ key, label, placeholder }) => (
            <div key={key}>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1.5">{label}</label>
              {key === 'problem' || key === 'campaign_info' ? (
                <textarea value={(form as any)[key]} onChange={e => set(key, e.target.value)} placeholder={placeholder} rows={3}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1.5">상금</label>
              <input type="text" value={form.reward} onChange={e => set('reward', e.target.value)} placeholder="예: ₩ 1,000,000"
                className={inputClass} />
            </div>
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1.5">마감</label>
              <input type="text" value={form.deadline} onChange={e => set('deadline', e.target.value)} placeholder="예: 7일 후 마감"
                className={inputClass} />
            </div>
          </div>

          <div>
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">카드 배경색</label>
            <div className="flex gap-2 flex-wrap">
              {BRIEF_BG_OPTIONS.map(opt => (
                <button key={opt.value} type="button" onClick={() => set('bg_color', opt.value)}
                  className={cn("w-9 h-9 rounded-xl transition-all border-2", opt.value,
                    form.bg_color === opt.value ? "border-black scale-110 shadow" : "border-transparent hover:scale-105")}>
                </button>
              ))}
            </div>
          </div>
...
function PostDetail() {
  ...
  const handleDeletePost = async () => {
    ...
    else { alert('삭제 중 오류가 발생했습니다.'); setConfirmDelete(false); }
  };
    ...
    <div className="min-h-screen bg-white pt-14">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="flex items-center justify-center gap-3 py-8 border-y border-gray-100 mb-10">
          <button onClick={() => handlePostVote('like')} disabled={!!voted}
            className={cn("flex items-center gap-2 px-6 py-3 rounded-2xl border-2 transition-all",
              voted === 'like'
                ? "border-[#05D560] text-[#05D560]"
                : "border-gray-200 text-gray-400 hover:border-gray-300 disabled:cursor-not-allowed")}>
            <ThumbsUp size={18} />
            <span className="text-sm font-black">추천 {post.likes}</span>
          </button>
          <button onClick={() => handlePostVote('dislike')} disabled={!!voted}
            className={cn("flex items-center gap-2 px-6 py-3 rounded-2xl border-2 transition-all",
              voted === 'dislike'
                ? "border-[#05D560] text-[#05D560]"
                : "border-gray-200 text-gray-400 hover:border-gray-300 disabled:cursor-not-allowed")}>
            <ThumbsDown size={18} />
            <span className="text-sm font-black">비추천 {post.dislikes ?? 0}</span>
          </button>
        </div>
