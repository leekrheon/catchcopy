...
// 마감일 문자열에서 날짜 파싱 후 실시간 D-day 계산
const getDday = (deadlineStr: string): { label: string; isExpired: boolean } => {
  // "2025-07-18 (D-30)" 또는 "2025-07-18" 형태에서 날짜 추출
  const dateMatch = deadlineStr.match(/(\d{4}-\d{2}-\d{2})/);
  if (!dateMatch) return { label: deadlineStr, isExpired: false };
  const deadline = new Date(dateMatch[1]);
  deadline.setHours(23, 59, 59, 999);
  const now = new Date();
  const diff = Math.ceil((deadline.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  if (diff < 0) return { label: '마감', isExpired: true };
  if (diff === 0) return { label: '오늘 마감', isExpired: false };
  if (diff === 1) return { label: '내일 마감', isExpired: false };
  return { label: 'D-' + diff, isExpired: false };
};
