...
function LoginPage() {
  ...
  const handleAdminLogin = async () => {
    if (!adminPw) return;
    try {
      const result = await callEdgeFunction('admin-auth', { action: 'login', password: adminPw });
      if (result.token) {
        const profile = {
          name: result.profile?.name ?? '관리자',
          avatar: 'AD',
        ...
        localStorage.setItem('cc_admin_token', result.token);
        navigate('/catchcopy/mypage');
      }
    } catch (err: any) {
      // Edge Function 미배포 시 로컬 비밀번호로 폴백
      const FALLBACK_PW = 'catchthecopy1!';
      if (adminPw === FALLBACK_PW) {
        const profile = { name: '관리자', avatar: 'AD' };
        setUser(profile);
        localStorage.setItem('cc_user', JSON.stringify(profile));
        localStorage.setItem('cc_admin', '1');
        localStorage.setItem('cc_admin_token', 'local-' + Date.now());
        navigate('/catchcopy/mypage');
      } else {
        alert('비밀번호가 틀렸습니다.');
      }
...
function MyPage() {
    ...
    try {
      ...
      reader.onload = (ev) => {
        ...
        if (isAdminMode()) {
          ...
          if (token) {
            callEdgeFunction('admin-auth', { action: 'update_profile', token, avatar_url: base64 }).catch(() => {});
          }
        }
        setSaving(false);
      };
  ...
  const handleSave = () => {
    ...
    setUser(updated);
    localStorage.setItem('cc_user', JSON.stringify(updated));
    // 관리자면 DB에 백그라운드로 동기화 (결과와 무관하게 즉시 이동)
    if (isAdminMode()) {
      const token = localStorage.getItem('cc_admin_token');
      if (token) {
        callEdgeFunction('admin-auth', { action: 'update_profile', token, display_name: updated.name }).catch(() => {});
      }
    }
    navigate(-1);
  };
  ...
  const handleLogout = async () => {
    const token = localStorage.getItem('cc_admin_token');
    if (token) {
      try { await callEdgeFunction('admin-auth', { action: 'logout', token }); } catch {}
    }
    setUser(null);
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {isWinner && (() => {
                              ...
                              const catches = [rewardAmount * 4, rewardAmount * 3, rewardAmount * 2, rewardAmount * 1][idx] ?? 0;
                              return (
                                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(251,191,36,0.15)', color: '#f59e0b' }}>
                                  {catches.toLocaleString()} 캐치 지급 예정
                                </span>
                              );
                            })()}
                          </div>
    ...
    <div className="min-h-screen bg-gray-50 pt-12">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-5" style={{ backgroundColor: dm ? D.card : '#f3f4f6' }}>
          {[{ id: 'briefs', label: '브리프 관리' }, { id: 'payouts', label: '지급대기 ' + payouts.filter(p => p.status === 'pending').length }, { id: 'ads', label: '광고 관리' }, { id: 'analytics', label: '분석' }].map(t => (
...
function BriefEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('briefs').update({
      ...
      reward_amount: (parseInt((form.reward ?? '').replace(/[^0-9]/g, '')) || 0),
...
const isAdminMode = () => {
  ...
  try {
    ...
    if (!userStr || !token || !lsAdmin) return false;
    const u = JSON.parse(userStr);
    return u?.name === '관리자' && u?.avatar === 'AD';
  } catch { return false; }
};
...
export default function App() {
  ...
  // 앱 시작 시 관리자 토큰 유효성 DB 검증
  useEffect(() => {
    const token = localStorage.getItem('cc_admin_token');
    const isAdmin = localStorage.getItem('cc_admin') === '1';
    if (token && isAdmin) {
      callEdgeFunction('admin-auth', { action: 'verify', token })
        .then(result => {
          if (result.valid === false && !token.startsWith('local-')) {
            // 명시적으로 invalid 응답 받을 때만 로그아웃
            localStorage.removeItem('cc_admin_token');
            localStorage.setItem('cc_admin', '0');
            setUser(null);
            localStorage.removeItem('cc_user');
          } else if (result.valid === true && result.profile) {
            // DB 최신 프로필로 동기화 (로컬과 다를 때만)
            const currentUser = JSON.parse(localStorage.getItem('cc_user') ?? '{}');
            const updated = {
              ...currentUser,
              name: result.profile.name ?? currentUser.name ?? '관리자',
              avatar: 'AD',
              avatarUrl: result.profile.avatarUrl ?? currentUser.avatarUrl ?? undefined,
            };
            setUser(updated);
            localStorage.setItem('cc_user', JSON.stringify(updated));
          }
        ...
        .catch(() => {
          // Edge Function 미배포 시 로컬 토큰이면 유지
        });
    }
  }, []);
