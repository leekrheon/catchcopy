        ...
        <div className="max-w-2xl mx-auto px-4 py-8 space-y-6" ref={contentRef}>
          ...
          {!hasRead && (
            <div className="rounded-xl px-4 py-3 flex items-center gap-3 border"
              style={{ backgroundColor: 'rgba(5,213,96,0.06)', borderColor: 'rgba(5,213,96,0.25)' }}>
              <span className="text-base">👇</span>
              <p className="text-xs font-medium leading-snug" style={{ color: KEY }}>
                브리프를 끝까지 읽은 후 하단 버튼을 눌러 카피 작성을 시작해주세요!
              </p>
            </div>
          )}
          {[
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              ...
              <div className="text-sm leading-relaxed" style={{ color: dm ? D.text : '#374151' }}
                dangerouslySetInnerHTML={{ __html: brief.problem ?? '' }} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>타겟</p>
                <div className="text-xs leading-relaxed" style={{ color: dm ? D.text : '#374151' }}
                  dangerouslySetInnerHTML={{ __html: brief.target ?? '' }} />
              </div>
              <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>캠페인</p>
                <div className="text-xs leading-relaxed" style={{ color: dm ? D.text : '#374151' }}
                  dangerouslySetInnerHTML={{ __html: brief.campaign_info ?? '' }} />
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex bg-gray-100 p-0.5 rounded-lg gap-0.5 shrink-0">
                  {([
                    ...
                    <button key={s.id} onClick={() => setBriefSortOrder(s.id)}
                      className={cn("px-3 py-1 rounded-md text-xs font-semibold transition-all whitespace-nowrap",
                        briefSortOrder === s.id ? "bg-white shadow-sm text-gray-800" : "text-gray-400")}>
                      {s.label}
                    </button>
                  ))}
                </div>
