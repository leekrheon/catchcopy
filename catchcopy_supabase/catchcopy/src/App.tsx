...
function MyPage() {
  ...
  const handleSave = async () => {
    ...
    if (isAdminMode()) {
      ...
      if (token) {
        try {
          await callEdgeFunction('admin-auth', { action: 'update_profile', token, display_name: updated.name });
        } catch {}
      }
    }
    navigate(-1);
  ...
  useEffect(() => {
    ...
    if (token && isAdmin) {
      callEdgeFunction('admin-auth', { action: 'verify', token })
        .then(result => {
          if (!result.valid && !token.startsWith('local-')) {
            localStorage.removeItem('cc_admin_token');
            localStorage.setItem('cc_admin', '0');
            setUser(null);
            localStorage.removeItem('cc_user');
          } else if (result.valid && result.profile) {
            // DB 최신 프로필로 동기화
            const updated = {
              name: result.profile.name ?? '관리자',
              avatar: 'AD',
