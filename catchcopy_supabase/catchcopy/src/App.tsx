...
function AdBanner() {
  ...
  useEffect(() => {
    ...
    timer.current = setInterval(() => setCurrent(p => (p + 1) % ads.length), 4000);
  ...
  const getObjPos = (a: Ad) => {
    const x = a.position_x ?? 'center';
    const y = a.position_y ?? 'center';
    return `${x} ${y}`;
  };

  return (
    // 2400:500 = 24:5 비율 고정 → 모바일/PC 동일 비율
                  ...
                  return (
                    <div key={ad.id} className="rounded-xl border overflow-hidden"
                      ...
                      <div className="px-3 py-2.5">
                        <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>이미지 위치 조정</p>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <p className="text-[10px] mb-1" style={{ color: D.muted }}>가로</p>
                            <div className="flex gap-1">
                              {['left', 'center', 'right'].map(v => (
                                <button key={v} onClick={() => handleAdPositionUpdate(ad.id, 'position_x', v)}
                                  className="flex-1 py-1 rounded-lg text-[10px] font-medium transition-all border"
                                  style={{
                                    backgroundColor: (ad.position_x ?? 'center') === v ? KEY : 'transparent',
                                    color: (ad.position_x ?? 'center') === v ? 'white' : D.muted,
                                    borderColor: (ad.position_x ?? 'center') === v ? KEY : (dm ? D.border : '#e5e7eb'),
                                  }}>
                          ...
                          <div>
                            <p className="text-[10px] mb-1" style={{ color: D.muted }}>세로</p>
                            <div className="flex gap-1">
                              {['top', 'center', 'bottom'].map(v => (
                                <button key={v} onClick={() => handleAdPositionUpdate(ad.id, 'position_y', v)}
                                  className="flex-1 py-1 rounded-lg text-[10px] font-medium transition-all border"
                                  style={{
                                    backgroundColor: (ad.position_y ?? 'center') === v ? KEY : 'transparent',
                                    color: (ad.position_y ?? 'center') === v ? 'white' : D.muted,
                                    borderColor: (ad.position_y ?? 'center') === v ? KEY : (dm ? D.border : '#e5e7eb'),
                                  }}>
                                  {v === 'top' ? '상' : v === 'center' ? '중' : '하'}
                                </button>
                              ))}
                            </div>
                          </div>
