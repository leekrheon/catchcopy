...
function BriefPage() {
  ...
  const handleSubmitCopy = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      if (uc) await supabase.from('user_catches').update({ catches: uc.catches + 400 }).eq('voter_key', vk);
      else await supabase.from('user_catches').insert({ voter_key: vk, catches: 400 });
      setCatches(catches + 400);
      localStorage.setItem('cc_catches', String(catches + 400));
      setHasCopy(true);
          ...
          {brief.external_url && (
            <a href={brief.external_url} target="_blank" rel="noopener noreferrer"
              ...
              <div>
                ...
                <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>브랜드 공식 페이지로 이동합니다</p>
              </div>
          ...
          <div className="max-w-2xl mx-auto">
            {!hasRead && lockSeconds === 0 ? (
              ...
              <div>
                ...
                <button disabled
                  className="w-full py-4 rounded-xl opacity-40 cursor-not-allowed flex items-center justify-center overflow-hidden"
                  style={{ backgroundColor: KEY, minHeight: 64 }}>
                  <img src="/button.png" alt="LET'S CATCHCOPY"
                    style={{ height: 28, width: 'auto', mixBlendMode: 'multiply', filter: 'invert(1)' }} />
                  <span className="text-white text-xs ml-2">({lockSeconds}초 후 활성화)</span>
                </button>
              </div>
            ) : (
              // 읽기 완료
              <button onClick={handleGoParticipate}
                className="w-full rounded-xl transition-all active:scale-95 hover:opacity-90 flex items-center justify-center overflow-hidden"
                style={{ backgroundColor: KEY, minHeight: 42 }}>
                <img src="/button.png" alt="LET'S CATCHCOPY"
                  style={{ height: 32, width: 'auto', mixBlendMode: 'multiply', filter: 'invert(1)' }} />
              </button>
            )}
          </div>
        </div>
...
// ── 게시글 상세 ───────────────────────────────────────────
// 관리자 모드: localStorage에 'cc_admin'='1' 이면 관리자
const isAdminMode = () => localStorage.getItem('cc_admin') === '1';
            ...
            <section className="border-b transition-colors overflow-hidden" style={{ minHeight: 'calc(100vh - 48px)', scrollSnapAlign: 'start', scrollSnapStop: 'always', backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              ...
              <div className="flex lg:hidden flex-col items-center justify-center text-center px-4 h-full" style={{ minHeight: 'calc(100vh - 48px)' }}>
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <button className="text-sm font-medium px-6 py-3 rounded-lg border hover:bg-gray-50 transition-all"
              ...
              <div className="hidden lg:flex items-stretch" style={{ minHeight: 'calc(100vh - 48px)' }}>
                ...
                <div className="flex-1 flex flex-col items-end justify-center pr-16 xl:pr-24 text-right">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button className="text-sm font-medium px-7 py-3 rounded-lg border hover:bg-gray-50 transition-all"
                      style={{ color: dm ? D.text : '#374151', borderColor: dm ? D.border : '#e5e7eb' }}>
                      더 알아보기
                    </button>
                  </div>
...
export default function App() {
  ...
  const [catches, setCatches] = useState<number>(() => {
    try { return parseInt(localStorage.getItem('cc_catches') ?? '0') || 0; } catch { return 0; }
  });
  ...
  useEffect(() => {
    ...
    supabase.from('user_catches').select('catches').eq('voter_key', vk).single()
      .then(({ data }) => { if (data) { setCatches(data.catches); localStorage.setItem('cc_catches', String(data.catches)); } });
  }, []);
