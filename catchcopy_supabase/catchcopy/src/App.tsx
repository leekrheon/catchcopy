...
function BriefPage() {
  ...
  const handleSubmitCopy = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    await supabase.from('copies').insert({ brief_id: briefId, author: user?.name ?? '익명', content: newCopy.trim(), upvotes: 0 });
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            ...
            {brief.status === '진행중' && (
              <div className="rounded-xl p-4 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-3" style={{ color: D.muted }}>내 카피 제출</p>
                <form onSubmit={handleSubmitCopy} className="relative">
                  <input type="text" value={newCopy} onChange={e => setNewCopy(e.target.value)}
                    ...
                    className="w-full rounded-lg px-3 py-2.5 pr-10 text-sm focus:outline-none focus:border-[#05D560] transition-colors border" style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }} />
                  <button type="submit"
    ...
    <div className="min-h-screen bg-gray-50 pt-12">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 mb-5 flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-gray-800">커뮤니티 관리자 모드</p>
            <p className="text-[11px] text-gray-400 mt-0.5">활성화 시 모든 게시글·댓글을 수정/삭제할 수 있습니다.</p>
          </div>
