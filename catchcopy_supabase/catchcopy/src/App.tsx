...
// ── 브리프 모달 ──────────────────────────────────────────
function BriefModal({ brief, onClose }: { brief: Brief; onClose: () => void }) {
  ...
  useEffect(() => {
    setLoading(true);
    supabase.from('copies').select('*').eq('brief_id', brief.id)
      .order('created_at', { ascending: false })
      .then(({ data }) => { setCopies(data ?? []); setLoading(false); });

    const channel = supabase.channel(`copies-${brief.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'copies', filter: `brief_id=eq.${brief.id}` },
  ...
  const handleSubmitCopy = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    await supabase.from('copies').insert({ brief_id: brief.id, author: '나', content: newCopy.trim(), upvotes: 0, downvotes: 0 });
    setNewCopy('');
    setSubmitting(false);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 30 }}
        ...
        <div className={cn("relative h-40 rounded-t-3xl", brief.bg_color)}>
          <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-transform">
            <X size={16} />
          </button>
          <div className="absolute bottom-4 left-6 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black text-gray-500 uppercase tracking-widest">{brief.category}</p>
              <p className="text-sm font-black text-gray-800">{brief.company_name}</p>
            </div>
          </div>
...
function AppInner() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-[#1a1a1a]">
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-10">
            <button onClick={() => { setActiveTab('home'); setSelectedBrief(null); setSelectedPostId(null); }} className="flex items-center">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {[{ id: 'brief', label: '브리프' }, { id: 'community', label: '커뮤니티' }].map(tab => (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); setSelectedPostId(null); setSelectedBrief(null); }}
            ...
            <section className="max-w-7xl mx-auto px-6 py-14">
              ...
              {briefsLoading ? <Spinner /> : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {homeBriefs.map(brief => <BriefCard key={brief.id} brief={brief} onClick={() => setSelectedBrief(brief)} />)}
                </div>
              )}
            </section>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-6 py-14">
            ...
            {briefsLoading ? <Spinner /> : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredBriefs.map(brief => <BriefCard key={brief.id} brief={brief} onClick={() => setSelectedBrief(brief)} />)}
                  ...
                  {postsLoading ? <Spinner /> : (
                    <div className="space-y-3">
                      {filteredPosts.length === 0 ? (
                        ...
                        <motion.div key={post.id} whileHover={{ x: 3 }} onClick={() => setSelectedPostId(post.id)}
      ...
      <AnimatePresence>
        {selectedBrief && <BriefModal brief={selectedBrief} onClose={() => setSelectedBrief(null)} />}
      </AnimatePresence>
