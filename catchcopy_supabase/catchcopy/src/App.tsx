...
interface Brief {
  ...
  bg_color: string; image_url?: string;
}
interface CopyEntry {
      ...
      <div className="max-w-6xl mx-auto px-4 h-12 flex items-center justify-between">
        <div className="flex items-center gap-5">
          ...
          <nav className="hidden md:flex items-center gap-0.5">
            {[{ id: 'brief', label: '브리프' }, { id: 'community', label: '커뮤니티' }].map(t => (
              <button key={t.id} onClick={() => navigate(`/catchcopy/${t.id}`)}
                className={cn("px-3 py-1.5 rounded-lg text-xs font-semibold transition-all",
                  activeTab === t.id ? "text-[#05D560] bg-[#05D560]/8" : "text-gray-500 hover:text-gray-900 hover:bg-gray-50")}>
                {t.label}
              </button>
            ))}
          </nav>
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => navigate('/catchcopy/admin/brief/write')}
            className="hidden sm:block text-xs font-medium text-gray-400 hover:text-gray-700 px-3 py-1.5 rounded-lg border border-gray-200 hover:border-gray-300 transition-all">
            관리자
          </button>
          <button className="hidden sm:block text-xs font-medium text-gray-500 hover:text-gray-900 px-3 py-1.5 transition-colors">로그인</button>
          <button onClick={() => navigate('/catchcopy/community/write')}
            className="hidden sm:flex items-center gap-1 text-xs font-semibold text-white px-3 py-1.5 rounded-lg transition-all hover:opacity-90"
            style={{ backgroundColor: KEY }}>
            <Edit3 size={11} /> 글쓰기
          </button>
          <button onClick={() => setMobileOpen(v => !v)} className="md:hidden w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100">
            ...
            <div className="pt-2 space-y-0.5">
              {[{ id: 'brief', label: '브리프' }, { id: 'community', label: '커뮤니티' }].map(t => (
                <button key={t.id} onClick={() => { navigate(`/catchcopy/${t.id}`); setMobileOpen(false); }}
                  className={cn("w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium",
                    activeTab === t.id ? "text-[#05D560] bg-[#05D560]/8" : "text-gray-600 hover:bg-gray-50")}>
                  {t.label}
                </button>
              ))}
              <button onClick={() => { navigate('/catchcopy/community/write'); setMobileOpen(false); }}
                className="w-full text-left px-3 py-2.5 rounded-lg text-sm font-semibold text-white flex items-center gap-2"
                style={{ backgroundColor: KEY }}>
                <Edit3 size={13} /> 글쓰기
              </button>
              <button onClick={() => { navigate('/catchcopy/admin/brief/write'); setMobileOpen(false); }}
                className="w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium text-gray-500 hover:bg-gray-50">
                관리자
              </button>
            </div>
          </motion.div>
...
function BriefPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('copy_votes').select('copy_id').eq('voter_key', vk)
      .then(({ data }) => { if (data) setMyVotes(new Set(data.map((r: any) => r.copy_id))); });
  ...
  const handleVote = async (copyId: string, currentUpvotes: number) => {
    const vk = getVoterKey();
    if (myVotes.has(copyId)) {
      // 취소
      await supabase.from('copy_votes').delete().eq('copy_id', copyId).eq('voter_key', vk);
      await supabase.from('copies').update({ upvotes: Math.max(0, currentUpvotes - 1) }).eq('id', copyId);
      ...
      if (!error) {
        await supabase.from('copies').update({ upvotes: currentUpvotes + 1 }).eq('id', copyId);
                  ...
                  {copies.map((copy, idx) => {
                    ...
                    return (
                      <motion.div key={copy.id} layout
                        ...
                        {/* 추천 버튼 */}
                        <button onClick={() => handleVote(copy.id, copy.upvotes)}
...
function BriefWritePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      ...
      const { error: uploadError } = await supabase.storage.from('brief-images').upload(path, imageFile, { upsert: true });
      if (!uploadError) {
    ...
    <div className="min-h-screen bg-gray-50 pt-12">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[{ key: 'reward', label: '상금', placeholder: '₩ 1,000,000' }, { key: 'deadline', label: '마감', placeholder: '7일 후 마감' }].map(({ key, label, placeholder }) => (
              <div key={key}>
                <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">{label}</label>
                <input type="text" value={(form as any)[key]} onChange={e => set(key, e.target.value)} placeholder={placeholder} className={inputClass} />
              </div>
...
function WritePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    if (isEdit) {
      await supabase.from('posts').update({ title: form.title.trim(), content: form.content.trim(), category: form.category }).eq('id', postId);
      ...
      <div className="sticky top-12 z-40 bg-white border-b border-gray-100">
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center justify-between">
          ...
          <button onClick={handleSubmit} disabled={!canSubmit}
            className="text-xs font-semibold text-white px-4 py-1.5 rounded-lg transition-all hover:opacity-90 disabled:opacity-30"
            style={{ backgroundColor: KEY }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
              className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 appearance-none focus:outline-none focus:border-[#05D560] transition-colors">
          ...
          <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
            placeholder="제목"
            className="w-full bg-transparent border-0 border-b border-gray-100 py-3 text-xl font-bold placeholder:text-gray-200 focus:outline-none focus:border-gray-300 transition-colors" />
          <textarea value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))}
            placeholder="내용을 작성하세요."
            className="w-full bg-transparent border-0 py-3 text-sm text-gray-700 placeholder:text-gray-300 focus:outline-none resize-none leading-relaxed"
        ...
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden mb-3">
          <div className="p-5 sm:p-6">
            ...
            <div className="py-5 space-y-0.5">
              {post.content.split('\n').map((line, i) => (
                <p key={i} className={cn("text-sm leading-relaxed text-gray-700", line === '' ? 'h-3' : '')}>{line}</p>
              ))}
            </div>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <main className="pt-12">
        ...
        {activeTab === 'home' && (
          <div>
            {/* 히어로 */}
            <section className="bg-white border-b border-gray-100 px-4 py-12 sm:py-16 text-center">
              <div className="max-w-2xl mx-auto">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#05D560]/10 mb-6">
                  <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: KEY }} />
                  <span className="text-xs font-semibold text-[#05D560]">실시간 경쟁 진행 중</span>
                </div>
                <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4 leading-tight">
                  단 한 줄로<br /><span className="text-[#05D560]">설득하라.</span>
                </h2>
                <p className="text-sm text-gray-500 mb-8 leading-relaxed">브랜드의 고민을 해결하는 한 줄의 카피.<br />지금 바로 도전하고 보상을 쟁취하세요.</p>
                <div className="flex items-center justify-center gap-2">
                  <button onClick={() => navigate('/catchcopy/brief')}
                    className="text-sm font-semibold text-white px-5 py-2.5 rounded-lg hover:opacity-90 transition-all active:scale-95"
                    style={{ backgroundColor: KEY }}>
                    브리프 보기
                  </button>
                  <button className="text-sm font-medium text-gray-600 px-5 py-2.5 rounded-lg border border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition-all">
                    더 알아보기
                  </button>
                </div>
              </div>
            ...
            <section className="bg-white border-b border-gray-100">
              <div className="max-w-6xl mx-auto px-4 py-5 grid grid-cols-3 divide-x divide-gray-100">
                {[
          ...
          <div className="max-w-4xl mx-auto px-4 py-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
              ...
              <aside className="hidden lg:block lg:col-span-1">
                <div className="bg-white rounded-xl border border-gray-100 p-4 lg:sticky lg:top-16">
                  ...
                  <div className="space-y-2">
                    {filteredPosts.length === 0 ? (
                      ...
                      <motion.div key={post.id} whileTap={{ scale: 0.995 }}
                        ...
                        className="bg-white rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm px-4 py-3.5 cursor-pointer transition-all">
      ...
      <footer className="border-t border-gray-100 bg-white py-6 mt-8">
        <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-3">
          <img src="/logo.png" alt="CatchCopy" className="h-4 w-auto object-contain" />
          <p className="text-[11px] text-gray-400">© 2025 CATCHCOPY. All rights reserved.</p>
