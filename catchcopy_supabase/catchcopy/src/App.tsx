...
function WritePage() {
  ...
  // editorRef가 마운트된 후 content 설정
  useEffect(() => {
    if (editContent && editorRef.current && !loading) {
  ...
  const handleSubmit = async () => {
    ...
    if (isEdit) {
      ...
      const { data } = await supabase.from('posts').insert({
        ...
        author: user?.name ?? '익명', avatar: user?.avatar ?? 'AN', views: 0, likes: 0, dislikes: 0, is_pinned: false,
      }).select().single();
