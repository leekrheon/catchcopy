...
function BriefCard({ brief, onClick, large }: { brief: Brief; onClick: () => void; large?: boolean }) {
  ...
  return (
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.99 }} onClick={onClick}
      className="cursor-pointer group rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6' }}>
      {/* 이미지 영역 */}
...
function BriefPage() {
  ...
  const [mode, setMode] = useState<'read' | 'participate'>('read'); // 읽기 / 참여
  ...
  const [hasVisitedUrl, setHasVisitedUrl] = useState(false); // 외부 URL 방문 여부
  const [lockSeconds, setLockSeconds] = useState(0);   // 남은 잠금 초
  ...
  useEffect(() => {
    ...
    supabase.from('brief_reads').select('id').eq('brief_id', briefId).eq('voter_key', vk).single()
      .then(({ data }) => { if (data) setHasRead(true); });
  }, [briefId]);
  ...
  const startReadLock = () => {
    ...
    lockTimer.current = setInterval(() => {
      ...
      if (elapsed >= secs) {
        ...
        setLockSeconds(0);
        setHasRead(true);
        supabase.from('brief_reads').insert({ brief_id: briefId, voter_key: vk }).catch(() => {});
      }
    }, 1000);
  };

  const handleGoParticipate = async () => {
    if (lockSeconds > 0) return;
    // DB에서 실제 읽음 기록 재확인 (state 조작 방어)
    const { data: readRecord } = await supabase.from('brief_reads')
      .select('id').eq('brief_id', briefId).eq('voter_key', vk).single();
    if (!readRecord) {
      alert('브리프를 먼저 읽어주세요.');
      return;
    }
  ...
  const handleSubmitCopy = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    // DB 레벨에서 중복 실시간 재확인 (state 조작 방어)
    const { data: existingCopy } = await supabase.from('copies')
      .select('id').eq('brief_id', briefId).eq('voter_key', vk).single();
    if (existingCopy) {
      setHasCopy(true);
      setSubmitting(false);
      return alert('이 브리프에는 이미 카피를 작성하셨습니다.');
    }
    const { error } = await supabase.from('copies').insert({
      brief_id: briefId, author: user?.name ?? '익명',
      content: newCopy.trim(), upvotes: 0, voter_key: vk
    });
    if (!error) {
      // DB 서버사이드 함수로 캐치 적립 (중복/변조 방지)
      const { data: result } = await supabase.rpc('award_catch_for_copy', {
        p_voter_key: vk,
        p_brief_id: briefId,
        p_catch_amount: 400,
      });
      const newCatches = result?.catches ?? (catches + 400);
  ...
  const handleVote = async (copyId: string) => {
    ...
    // 자기 카피엔 투표 불가
    const ownCopy = copies.find(c => c.id === copyId && c.voter_key === vk);
    if (ownCopy) return alert('자신의 카피에는 투표할 수 없습니다.');
    if (alreadyVoted) {
          ...
          <div className="rounded-2xl p-5 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>{brief.deadline}</p>
              </div>
              <div>
          ...
          <div className="max-w-2xl mx-auto space-y-2">
            {!hasRead && lockSeconds === 0 ? (
              ...
              <div>
                ...
                <button disabled
                  className="w-full py-4 rounded-xl opacity-40 cursor-not-allowed flex items-center justify-center gap-2"
                  style={{ backgroundColor: KEY }}>
            ...
            ) : brief.external_url && !hasVisitedUrl ? (
              // external_url 있는데 아직 방문 안 했으면 잠금
              <div className="space-y-2">
                <p className="text-center text-xs" style={{ color: D.muted }}>
                  아래 버튼을 눌러 브리프 상세 페이지를 확인 후 카피를 작성할 수 있어요.
                </p>
                <button disabled
              ...
              <button onClick={handleGoParticipate}
                className="w-full py-4 rounded-xl text-sm font-bold transition-all active:scale-95 hover:opacity-90 flex items-center justify-center"
                style={{ backgroundColor: KEY }}>
                <img src="/button.png" alt="LET'S CATCHCOPY" style={{ height: 22, width: 'auto' }} />
              </button>
            )}
            ...
            <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold" style={{ color: dm ? D.text : '#1f2937' }}>실시간 경쟁 랭킹</span>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  {copies.map((copy, idx) => {
                    ...
                    return (
                      <motion.div key={copy.id} layout
                        ...
                        style={{ backgroundColor: idx === 0 ? 'rgba(5,213,96,0.03)' : 'transparent' }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>{copy.author}</p>
                        </div>
...
function WalletPage() {
  const navigate = useNavigate();
  const { darkMode: dm, catches, user } = useContext(AppContext);
  const [history, setHistory] = useState<any[]>([]);
  const vk = getVoterKey();

  useEffect(() => {
    supabase.from('catch_payouts').select('*, briefs(title)').eq('voter_key', vk).order('created_at', { ascending: false })
      .then(({ data }) => setHistory(data ?? []));
  }, []);

  return (
    <div className="min-h-screen pt-12 pb-20 lg:pb-0" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      <div className="max-w-lg mx-auto px-4 py-6">
        {/* 잔액 카드 */}
        <div className="rounded-2xl p-6 mb-5 text-white" style={{ backgroundColor: KEY }}>
          ...
          <div className="mt-4 pt-4 border-t border-white/20 grid grid-cols-2 gap-3 text-center">
            <div>
              <p className="text-xl font-bold">{Math.floor(catches / 400)}</p>
              <p className="text-[11px] opacity-70">카피 작성</p>
            </div>
            <div>
              <p className="text-xl font-bold">{history.filter(h => h.status === 'paid').length}</p>
              <p className="text-[11px] opacity-70">TOP4 수상</p>
            </div>
          </div>
        </div>
        ...
        <button onClick={() => navigate('/catchcopy/market')}
          className="w-full rounded-xl p-4 mb-5 flex items-center justify-between border transition-all"
          style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        ...
        {history.length > 0 && (
          <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
            <div className="px-4 py-3 border-b" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
              <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>TOP4 수상 내역</p>
            </div>
            {history.map((h, i) => (
              <div key={h.id} className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                <div>
                  <p className="text-xs font-semibold" style={{ color: dm ? D.text : '#111' }}>{h.rank}등 · {h.briefs?.title ?? '브리프'}</p>
                  <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>{new Date(h.created_at).toLocaleDateString('ko-KR')}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold" style={{ color: KEY }}>+{h.catch_amount.toLocaleString()}</p>
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium"
                    style={{ backgroundColor: h.status === 'paid' ? 'rgba(5,213,96,0.1)' : 'rgba(251,191,36,0.1)', color: h.status === 'paid' ? KEY : '#f59e0b' }}>
                    {h.status === 'paid' ? '지급완료' : '지급대기'}
                  </span>
                </div>
              </div>
            ))}
        ...
        {history.length === 0 && (
          <div className="text-center py-12" style={{ color: D.muted }}>
            ...
            <p className="text-xs">브리프에 카피를 작성하고 캐치를 모아보세요!</p>
            <p className="text-[11px] mt-1">카피 작성 시 400 캐치 적립</p>
          </div>
        )}
      </div>
    </div>
...
function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'briefs' | 'payouts'>('briefs');
    ...
    <div className="min-h-screen bg-gray-50 pt-12">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-5" style={{ backgroundColor: dm ? D.card : '#f3f4f6' }}>
          {[{ id: 'briefs', label: '브리프 관리' }, { id: 'payouts', label: '지급대기 ' + payouts.filter(p => p.status === 'pending').length }].map(t => (
                          ...
                          {hotPosts.map((post, idx) => (
                            <button key={post.id} onClick={() => navigate(`/catchcopy/community/post/${post.id}`)}
                              ...
                              <span className="text-[11px] font-bold shrink-0 w-4 mt-0.5"
                                style={{ color: idx === 0 ? KEY : idx === 1 ? '#9ca3af' : idx === 2 ? '#f97316' : '#d1d5db' }}>
                                {idx + 1}
                              </span>
