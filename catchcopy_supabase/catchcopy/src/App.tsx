          ...
          {[
            {
              id: 'home', label: '홈',
              icon: (active: boolean) => (
                <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? KEY : (dm ? D.muted : '#9ca3af')}>
            ...
            <button key={t.id} onClick={() => navigate(`/catchcopy/${t.id === 'home' ? '' : t.id}`)}
              className="flex flex-col items-center justify-center gap-0.5 transition-colors">
              {t.icon(activeTab === t.id)}
              <span className="text-[9px] font-medium" style={{ color: activeTab === t.id ? KEY : (dm ? D.muted : '#9ca3af') }}>{t.label}</span>
            </button>
          ))}
        </div>
      </nav>
