...
function BriefCard({ brief, onClick, large }: { brief: Brief; onClick: () => void; large?: boolean }) {
  ...
  return (
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.99 }} onClick={onClick}
      className="cursor-pointer group rounded-2xl overflow-hidden transition-all" style={{ backgroundColor: dm ? D.card : 'white', border: `1px solid ${dm ? D.border : '#f3f4f6'}` }}>
      {/* 이미지 영역 */}
                    ...
                    return (
                      <motion.div key={copy.id} layout
                        className="px-4 py-3.5 flex items-start gap-3 transition-colors"
                          style={{ backgroundColor: idx === 0 ? `${KEY}08` : 'transparent' }}>
                        {/* 순위 */}
                        ...
                        <button onClick={() => handleVote(copy.id)}
                          ...
                          style={{
                            border: `1px solid ${voted ? KEY : (dm ? D.border : '#e5e7eb')}`,
                            color: voted ? KEY : '#9ca3af',
                            backgroundColor: voted ? `${KEY}0d` : 'transparent',
                          }}>
          ...
          <div className="p-5 sm:p-6">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-50">
              ...
              <div className="flex items-center gap-3">
                ...
                {(isAdminMode() || user?.name === post.author) && (<>
                ...
                <div key={comment.id} className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {/* 본인 댓글이거나 관리자만 수정/삭제 */}
                      {(isAdminMode() || user?.name === comment.author) && (
                        <div className="flex items-center gap-2">
                    ...
                    <div className="space-y-0.5">
                      {CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => setActiveCategory(cat)}
                          ...
                          style={{ color: activeCategory === cat ? KEY : dm ? D.muted : '#6b7280', backgroundColor: activeCategory === cat ? `${KEY}15` : 'transparent' }}>
                          {cat}
                        </button>
                      ))}
                    </div>
