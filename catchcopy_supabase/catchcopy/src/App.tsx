          ...
          <div className="max-w-2xl mx-auto">
            {!hasRead && lockSeconds === 0 ? (
              ...
              <div>
                ...
                <button disabled
                  className="w-full py-4 rounded-xl text-sm font-bold text-white opacity-40 cursor-not-allowed"
                  style={{ backgroundColor: KEY }}>
                  카피 작성하러 가기 ({lockSeconds}초 후 활성화)
                </button>
              </div>
            ) : (
              // 읽기 완료
              <button onClick={handleGoParticipate}
                className="w-full py-4 rounded-xl text-sm font-bold text-white transition-all active:scale-95 hover:opacity-90"
                style={{ backgroundColor: KEY }}>
                ✍️ 카피 작성하러 가기
              </button>
            )}
