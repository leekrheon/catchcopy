...
function BriefPage() {
  ...
  const [hasRead, setHasRead] = useState(false);       // 브리프 읽음 여부
  const [hasCopy, setHasCopy] = useState(false);       // 이 브리프에 이미 카피 썼는지
  const [readProgress, setReadProgress] = useState(0); // 읽기 진행도 0~100
  ...
  useEffect(() => {
    ...
    const vkLocal = vk;
    supabase.from('copy_votes').select('copy_id').eq('voter_key', vkLocal)
      .then(({ data }) => {
  ...
  // 브리프 읽기 타이머 (10초 이상 체류 시 읽은 것으로 처리)
  useEffect(() => {
    if (hasRead || briefLoading) return;
    let elapsed = 0;
    readTimer.current = setInterval(() => {
      elapsed += 1;
      setReadProgress(Math.min(100, elapsed * 10));
      if (elapsed >= 10) {
        clearInterval(readTimer.current);
        setHasRead(true);
        supabase.from('brief_reads').insert({ brief_id: briefId, voter_key: vk }).then(() => {});
      }
    }, 1000);
    return () => clearInterval(readTimer.current);
  }, [briefLoading, hasRead]);

  const handleSubmitCopy = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCopy.trim() || submitting) return;
    if (!hasRead) return alert('브리프를 먼저 충분히 읽어주세요!');
    ...
    if (!error) {
      ...
      if (uc) {
        await supabase.from('user_catches').update({ catches: uc.catches + 400 }).eq('voter_key', vk);
      } else {
                ...
                {!hasRead && (
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: D.muted }}>브리프 읽는 중...</p>
                      <span className="text-[10px]" style={{ color: KEY }}>{readProgress}%</span>
                    </div>
                    <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: dm ? D.hover : '#f3f4f6' }}>
                      <div className="h-full rounded-full transition-all duration-1000" style={{ width: readProgress + '%', backgroundColor: KEY }} />
                    </div>
                    <p className="text-[10px] mt-1" style={{ color: D.muted }}>브리프를 읽고 나면 카피 작성 및 투표가 가능합니다.</p>
                  </div>
                )}
                {hasRead && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(5,213,96,0.1)', color: KEY }}>
                        +400 캐치 적립
                      </span>
                    </div>
                    {hasCopy ? (
              ...
              <div className="hidden lg:flex items-stretch" style={{ minHeight: 'calc(100vh - 48px)' }}>
                ...
                <div className="flex-1 flex flex-col items-end justify-center pr-16 xl:pr-24 text-right">
                  <div className="flex flex-col items-end gap-3 mb-6">
                    ...
                    {/* 이미지 아래 키컬러 언더라인 */}
                    <div className="h-1.5 rounded-full" style={{ width: 380, backgroundColor: KEY }} />
                  </div>
